...
#include "BeJwrMSq2SM7PK0nNKykuNTTwycxL9SvNTUotAgB6ZgmH.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring contents,uint32 LineNumber>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("LineNumber");
        TypeOffset t(MY_OFFSETOF(LineNumber_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "LineNumber") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, LineNumber_);
   else
     istr >> LineNumber_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, LineNumber_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "LineNumber=", get_LineNumber()) ;
    ostr << '}';
}
