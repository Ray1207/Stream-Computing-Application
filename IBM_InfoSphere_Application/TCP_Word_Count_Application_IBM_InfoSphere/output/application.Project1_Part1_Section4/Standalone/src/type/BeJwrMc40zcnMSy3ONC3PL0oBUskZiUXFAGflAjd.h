// t3i5linesi5wordsi5chars


#ifndef BEJWRMC40ZCNMSY3ONC3PL0OBUSKZIUXFAGFLAJD_H_
#define BEJWRMC40ZCNMSY3ONC3PL0OBUSKZIUXFAGFLAJD_H_
...
#define SELF BeJwrMc40zcnMSy3ONC3PL0oBUskZiUXFAGflAjd
...
public:
    ...
    typedef SPL::int32 lines_type;
    typedef SPL::int32 words_type;
    typedef SPL::int32 chars_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), lines_(ot.lines_), words_(ot.words_), chars_(ot.chars_) 
      { constructPayload(ot); }
    SELF(const lines_type & _lines, const words_type & _words, const chars_type & _chars) : Tuple(), lines_(_lines), words_(_words), chars_(_chars) { }
    ...
    lines_type & get_lines() { return lines_; }
    const lines_type & get_lines() const { return lines_; }
    void set_lines(const lines_type & _lines) { lines_ = _lines; }
    
    words_type & get_words() { return words_; }
    const words_type & get_words() const { return words_; }
    void set_words(const words_type & _words) { words_ = _words; }
    
    chars_type & get_chars() { return chars_; }
    const chars_type & get_chars() const { return chars_; }
    void set_chars(const chars_type & _chars) { chars_ = _chars; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<lines_type >()(lines_);
        s = 37 * s + std::tr1::hash<words_type >()(words_);
        s = 37 * s + std::tr1::hash<chars_type >()(chars_);
        return s;
    }
    
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::int32)+sizeof(SPL::int32)+sizeof(SPL::int32);
    ...
    void swap(SELF & ot) 
    { 
        std::swap(lines_, ot.lines_);
        std::swap(words_, ot.words_);
        std::swap(chars_, ot.chars_);
...
#undef SELF
#endif // BEJWRMC40ZCNMSY3ONC3PL0OBUSKZIUXFAGFLAJD_H_ 
