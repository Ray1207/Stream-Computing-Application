...
#include "BeJwrMXQzNCjOzM3MSSzKLKkEACi6AWU.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<float64 similarity>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    
    {
        ...
        TypeOffset t(MY_OFFSETOF(similarity_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "similarity") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, similarity_);
   else
     istr >> similarity_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, similarity_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "similarity=", get_similarity()) ;
    ostr << '}';
}
