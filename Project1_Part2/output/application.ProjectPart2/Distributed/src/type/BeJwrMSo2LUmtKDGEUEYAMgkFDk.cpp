...
#include "BeJwrMSo2LUmtKDGEUEYAMgkFDk.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring text1,rstring text2>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("text1");
        TypeOffset t(MY_OFFSETOF(text1_, tm), 
    ...
    {
        std::string s("text2");
        TypeOffset t(MY_OFFSETOF(text2_, tm), 
                     Meta::Type::typeOf<SPL::rstring >(), 
                     &typeid(SPL::rstring));
        addMapping(*tm, t, s, 1);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "text1") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, text1_);
   else
     istr >> text1_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "text2") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, text2_);
   else
     istr >> text2_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, text1_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, text2_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "text1=", get_text1()) << ",";
    SPL::serializeWithPrecision(ostr << "text2=", get_text2()) ;
    ostr << '}';
}
