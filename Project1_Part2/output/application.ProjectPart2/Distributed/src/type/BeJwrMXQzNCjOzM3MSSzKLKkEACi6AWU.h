...
#ifndef BEJWRMXQZNCJOZM3MSSZKLKKEACI6AWU_H_
#define BEJWRMXQZNCJOZM3MSSZKLKKEACI6AWU_H_
...
#define SELF BeJwrMXQzNCjOzM3MSSzKLKkEACi6AWU
...
public:
    ...
    typedef SPL::float64 similarity_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), similarity_(ot.similarity_) 
      { constructPayload(ot); }
    SELF(const similarity_type & _similarity) : Tuple(), similarity_(_similarity) { }
    ...
    similarity_type & get_similarity() { return similarity_; }
    const similarity_type & get_similarity() const { return similarity_; }
    void set_similarity(const similarity_type & _similarity) { similarity_ = _similarity; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<similarity_type >()(similarity_);
        return s;
    }
    ...
    void swap(SELF & ot) 
    { 
        std::swap(similarity_, ot.similarity_);
...
#undef SELF
#endif // BEJWRMXQZNCJOZM3MSSZKLKKEACI6AWU_H_ 
