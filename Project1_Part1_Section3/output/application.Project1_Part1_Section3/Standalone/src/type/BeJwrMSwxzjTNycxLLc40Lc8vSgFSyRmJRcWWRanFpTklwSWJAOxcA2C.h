...
#ifndef BEJWRMSWXZJTNYCXLLC40LC8VSGFSYRMJRCWWRANFPTKLWSWJAOXCA2C_H_
#define BEJWRMSWXZJTNYCXLLC40LC8VSGFSYRMJRCWWRANFPTKLWSWJAOXCA2C_H_
...
#include "BeJwrMc40zcnMSy3ONC3PL0oBUskZiUXFAGflAjd.h"
...
public:
    ...
    typedef SPL::BeJwrMc40zcnMSy3ONC3PL0oBUskZiUXFAGflAjd resultSta_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), resultSta_(ot.resultSta_) 
      { constructPayload(ot); }
    SELF(const resultSta_type & _resultSta) : Tuple(), resultSta_(_resultSta) { }
    ...
    resultSta_type & get_resultSta() { return resultSta_; }
    const resultSta_type & get_resultSta() const { return resultSta_; }
    void set_resultSta(const resultSta_type & _resultSta) { resultSta_ = _resultSta; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<resultSta_type >()(resultSta_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += resultSta_.getSerializedSize();
    ...
    void swap(SELF & ot) 
    { 
        std::swap(resultSta_, ot.resultSta_);
...
#undef SELF
#endif // BEJWRMSWXZJTNYCXLLC40LC8VSGFSYRMJRCWWRANFPTKLWSWJAOXCA2C_H_ 
